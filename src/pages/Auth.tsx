...
const APP_SCHEME = 'app.lovable.acf26c10efdf4a99a1a925b44588ec52';
...
const Auth = () => {
  ...
  const startGoogleSignIn = async () => {
    ...
    setLoading(true);
    try {
      // Use the native app scheme for redirection to handle the auth response within the app
    ...
    try {
      if (isLogin) {
        ...
        const { error } = await supabase.auth.signUp({
          email,
          password,
          options: { emailRedirectTo: `${APP_SCHEME}://auth` },
        });
        if (error) throw error;
    ...
    <div className="min-h-screen flex items-center justify-center p-4 bg-background">
      ...
      >
        ...
        <button
          ...
          disabled={loading}
          className="w-full py-3 rounded-xl text-sm font-medium flex items-center justify-center gap-2 border border-border hover:border-primary/50 transition-colors disabled:opacity-50"
